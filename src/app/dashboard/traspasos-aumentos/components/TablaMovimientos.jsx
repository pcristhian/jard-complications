...
export default function TablaMovimientos({ sucursalSeleccionada, refreshTrigger, onMovimientoAnulado }) {
    ...
    // ✅ Estado para la lista de meses disponibles
    ...
    // ✅ Generar lista de meses disponibles desde el primer movimiento
    const generarMesesDisponibles = async () => {
        try {
            ...
            if (data && data.length > 0) {
                const fechaPrimerMovimiento = new Date(data[0].created_at);
                const hoy = new Date();
                const meses = [];

                let fecha = new Date(fechaPrimerMovimiento.getFullYear(), fechaPrimerMovimiento.getMonth(), 1);
                const hoyMes = new Date(hoy.getFullYear(), hoy.getMonth(), 1);

                while (fecha <= hoyMes) {
                    meses.push({
                        valor: `${fecha.getFullYear()}-${String(fecha.getMonth() + 1).padStart(2, '0')}`,
                        nombre: fecha.toLocaleString('es', { month: 'long', year: 'numeric' })
                    });
                ...
                // Si no hay movimientos, mostrar solo el mes actual
                const hoy = new Date();
                setMesesDisponibles([{
                    valor: `${hoy.getFullYear()}-${String(hoy.getMonth() + 1).padStart(2, '0')}`,
                    nombre: hoy.toLocaleString('es', { month: 'long', year: 'numeric' })
                }]);
            }
        } catch (err) {
            console.error('Error al generar meses disponibles:', err);
        }
    };
    ...
    }, [sucursalSeleccionada, refreshTrigger, filtroTipo, filtroEstado, mesSeleccionado]);
    ...
    const cargarMovimientos = async () => {
        ...
        try {
            ...
            query = query
                ...
                .lte('created_at', `${finStr}T23:59:59.999Z`);
    ...
    return (
        <>
            ...
            >
                    ...
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        <div>
                            ...
                            <p className="text-sm text-gray-600 mt-1">
                                {movimientosAgrupados.length} días con actividad · {movimientosAgrupados.reduce((sum, dia) => sum + dia.movimientos.length, 0)} movimientos
                            </p>
                        </div>
                        ...
                        <div className="flex flex-col sm:flex-row items-center gap-3">
                            ...
                            <div className="relative">
                                <input
                                    ...
                                    className="pl-10 pr-10 py-1.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 w-62 text-sm"
                                />
                            ...
                            <select
                                ...
                                className="px-3 py-1.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 text-sm bg-white"
                            >
                            ...
                            {/* ✅ Selector de Mes Minimalista - Misma altura que los demás filtros */}
                                ...
                                <div className="flex items-center gap-1">
                                    ...
                                    <div className="relative">
                                        <select
                                            ...
                                            className="appearance-none bg-white border border-gray-300 text-gray-700 text-sm rounded-lg px-3 py-1.5 pr-8 focus:outline-none focus:border-gray-400 focus:ring-1 focus:ring-gray-300 cursor-pointer"
                                        >
                            ...
                            <div className="flex items-center gap-1 bg-gray-100 rounded-lg p-0.5">
                                <button
                                    ...
                                    className={`px-3 py-1 rounded-md text-sm font-medium transition-all flex items-center gap-1 ${filtroEstado === 'activo'
                                ...
                                <button
                                    ...
                                    className={`px-3 py-1 rounded-md text-sm font-medium transition-all flex items-center gap-1 ${filtroEstado === 'anulado'
                ...
                {/* Lista agrupada por días con scroll */}
                        ...
                        {movimientosAgrupados.length === 0 ? (
                            ...
                            movimientosAgrupados.map((dia) => (
                                <div key={dia.fecha} className="bg-white">
                                    {/* Cabecera del día */}
                                    <button
                                        onClick={() => setDiasExpandidos(prev => ({ ...prev, [dia.fecha]: !prev[dia.fecha] }))}
                                                    ...
                                                    <table className="min-w-full divide-y divide-gray-200">
                                                        <thead className="bg-gray-50">
                                                            <tr>
                                                                ...
                                                                <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Sucursal</th>
                                                        ...
                                                        <tbody className="bg-white divide-y divide-gray-100">
                                                            {dia.movimientos.map((movimiento, idx) => {
                                                                ...
                                                                return (
                                                                    ...
                                                                    >
                                                                        <td className="px-4 py-3 whitespace-nowrap text-xs text-gray-500">{hora}</td>
                                                                        ...
                                                                        <td className="px-4 py-3 whitespace-nowrap text-center">
                                                                            <div className={`text-sm font-semibold ${movimiento.tipo_movimiento === 'entrada_stock' ? 'text-emerald-600' : 'text-blue-600'} ${esAnulado ? 'line-through' : ''}`}>
                                                                                {movimiento.tipo_movimiento === 'entrada_stock' ? '+' : ''}{movimiento.cantidad}
                                                                            </div>
                                                                        </td>
                ...
                {movimientosAgrupados.length > 0 && (
                    <div className="flex-shrink-0 px-6 py-3 bg-gray-50 border-t border-gray-200">
                        <div className="flex justify-between items-center text-sm text-gray-600">
                            <div>
                                Total: {movimientosAgrupados.reduce((sum, dia) => sum + dia.movimientos.length, 0)} movimientos
                            ...
                            <div className="flex gap-4">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span>Entradas: {movimientosAgrupados.reduce((sum, dia) => sum + dia.totalEntradas, 0)} uds</span>
                                </div>
                                <div className="flex items-center gap-2">
                                    <div className="w-3 h-3 rounded-full bg-blue-500"></div>
                                    <span>Traspasos: {movimientosAgrupados.reduce((sum, dia) => sum + dia.totalTraspasos, 0)} uds</span>
                                </div>
                            </div>
                        </div>
                    </div>
