...
export default function EditarSucursal({
    ...
    useEffect(() => {
        if (sucursal) {
            setFormData({
                ...
                activo: sucursal.activo === undefined ? true : !!sucursal.activo
            });
        }
    }, [sucursal]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const result = await updateSucursal(sucursal.id, formData);
            if (result.success) {
                ...
                throw new Error(result.error || 'Error actualizando sucursal');
            }
        } catch (err) {
                        ...
                        >
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                                ...
                                <motion.div variants={itemVariants}>
                                    ...
                                    <motion.input
                                        ...
                                        required
                                        className="w-full px-4 py-3 bg-gray-50 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent text-gray-900 transition-all duration-200 placeholder:text-gray-500"
                            ...
                            >
                                <motion.button
                                    ...
                                    whileTap={{ scale: 0.95 }}
                                    onClick={onCancelar}
                                    className="px-6 py-3 text-gray-700 bg-white cursor-pointer border border-gray-300 rounded-xl hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-gray-500 transition-all duration-200 font-medium"
                                >
                                    Cancelar
                                </motion.button>
                                <motion.button
